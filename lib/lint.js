...
function lintScopeDirectory(xdrsRoot, scopeName, errors, actualTypeIndexes, ignoreReadOnly) {
  ...
  for (const entry of entries) {
    ...
    if (entry.isDirectory()) {
      ...
      if (existsFile(typeIndexPath) && !shouldSkipReadOnlyPath(typeIndexPath, ignoreReadOnly)) {
  ...
  } else if (!shouldSkipReadOnlyPath(scopeIndexPath, ignoreReadOnly)) {
...
function lintSkillsDirectory(xdrsRoot, scopeName, typeName, subjectName, skillsPath, errors, ignoreReadOnly) {
  ...
  for (const entry of entries) {
    ...
    lintOrphanAssets(path.join(entryPath, RESOURCE_DIR_NAME), [skillFilePath], xdrsRoot, errors, ignoreReadOnly);
  }
...
function lintArticlesDirectory(xdrsRoot, scopeName, typeName, subjectName, articlesPath, errors, ignoreReadOnly) {
  ...
  lintOrphanAssets(path.join(articlesPath, RESOURCE_DIR_NAME), artifacts, xdrsRoot, errors, ignoreReadOnly);
...
function lintResearchDirectory(xdrsRoot, scopeName, typeName, subjectName, researchPath, errors, ignoreReadOnly) {
  ...
  lintOrphanAssets(path.join(researchPath, RESOURCE_DIR_NAME), artifacts, xdrsRoot, errors, ignoreReadOnly);
...
function lintPlansDirectory(xdrsRoot, scopeName, typeName, subjectName, plansPath, errors, ignoreReadOnly) {
  ...
  lintOrphanAssets(path.join(plansPath, RESOURCE_DIR_NAME), artifacts, xdrsRoot, errors, ignoreReadOnly);
...
function lintOrphanAssets(assetsDir, documentPaths, xdrsRoot, errors, ignoreReadOnly) {
  ...
  const assetFiles = entries
    ...
    .filter((entry) => !shouldSkipReadOnlyPath(path.join(assetsDir, entry.name), ignoreReadOnly))
